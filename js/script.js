...
function livelyPropertyListener(name, val) {
  switch (name) {
    ...
    case "backgroundSrc":
      backgroundSrc = val;
      if (isBackground) setBackground(val);
      break;
    case "backgroundFit":
...
function setOverlay(srcPath) {
  if (srcPath == undefined || srcPath == null) return;
...
function toggleOverlay(val) {
  document.getElementById("overlay").style.visibility = !val
    ? "hidden"
    : "visible";
}
...
function setBackgroundFit(index) {
  document.getElementById("videoBackground").style.objectFit = [
    "contain",
    "cover",
    "fill",
    "none",
...
function generateColor() {
  // Hex color #e0895d ko RGB mein convert karein
  let hex = "#e0895d";
  let rgb = hexToRgb(hex);

  // RGB values ko normalize karein (0 se 1 ke beech mein)
  let color = {
    r: rgb.r / 255,
    g: rgb.g / 255,
    b: rgb.b / 255,
